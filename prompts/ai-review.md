---
description: "Review the locally checked out code"
---

# Code Review Instructions

Perform a code review for the locally checked out code.

## Getting the diff

In order to get the diff to review, prefer the following methods (in priority order):

### Check the gh cli

Fetch the diff using `gh pr diff`.
This will return a reviewable diff if there is an open GitHub PR.

### Compare against the remote

- Refresh remote refs: `git fetch --all --prune`
- Resolve the remote base branch name (preferred order):
  - If no PR is found, get the repository default branch from `upstream`:

    ```bash
    base=$(git remote show upstream 2>/dev/null | sed -n '/HEAD branch/s/.*: //p')
    ```

  - Fallback to `origin` if `upstream` is unavailable:
...
- Diff the current branch against the resolved remote base (preferred):
  - Summary: `git diff --stat upstream/$base...HEAD` (or `git diff --stat origin/$base...HEAD` if using `origin`)
  - Full patch: `git diff upstream/$base...HEAD`
  - Changed files: `git diff --name-only upstream/$base...HEAD`
  - Unified diff for review comments: `git --no-pager diff --unified=3 upstream/$base...HEAD`

Optional: use `gh` for PR metadata and checkout

- If you want PR context (title, description, base branch, CI status), use the `gh` CLI:
  - Detect PR for current branch: `branch=$(git rev-parse --abbrev-ref HEAD); gh pr list --state open --head "$branch" --json number,baseRefName`
  - View PR details: `gh pr view <number> --json number,title,body,baseRefName,headRefName,headRepository,url`
- Prefer `gh` lookups only when you need PR metadata, not for a straightforward local diff.
